...
for sample_number in sample_number_list:
    ...
    # Model architecture
    def silhouette_input_cnn(input_shape):
        ...
        z = Dense(128, activation="relu")(x2)
        ...
        model = Model(inputs=silhouette_input, outputs=z)
    ...
    silhouette_path_sample = core_path+'Test-deep_learn/cnn_silhouette/sample_'+str(sample_number)
    model_save_location = silhouette_path_sample+'/normal_boosted_models/'  # specify your path
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = silhouette_input_cnn(input_shape)
