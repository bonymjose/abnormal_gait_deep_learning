...
for sample_number in sample_number_list:
    ...
    for class_name, patho_folder_names in class_dirs.items():
        for folder in patho_folder_names:
            ...
            for participant_number_folder in sorted(os.listdir(folder_dir)):
                ...
                # Access the 'skeleton' participant_number_folders
    ...
    # Model architecture
    def skeletal_input_cnn(input_shape):
        # input: Skeleton
        ...
        z = Dense(128, activation="relu")(x1)
        ...
        model = Model(inputs=skeleton_input, outputs=z)
    ...
    skeletal_path_sample = core_path+'Test-deep_learn/cnn_skeletal/sample_'+str(sample_number)
    model_save_location = skeletal_path_sample+'/normal_boosted_models/'  # specify path
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = skeletal_input_cnn(input_shape)
