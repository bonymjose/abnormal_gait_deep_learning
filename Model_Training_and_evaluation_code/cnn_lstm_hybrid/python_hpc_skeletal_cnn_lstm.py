...
from tensorflow.keras.layers import LSTM, Reshape, Dense, Flatten, TimeDistributed
...
for sample_number in sample_number_list:
    ...
    # Model architecture
    def skeletal_input_hybrid(input_shape):
    # Input Layer
        skeletal_input = Input(shape=input_shape, name='skeletal_input')
    
        # TimeDistributed wrapper to apply the same CNN to each time step in the sequence
        x = TimeDistributed(Conv2D(32, (3, 3), activation='relu'))(skeletal_input)
        ...
        # Create Model
        model = Model(inputs=skeletal_input, outputs=output)
    ...
    skeletal_path_sample = core_path+'Test-deep_learn/hybrid_skeletal/sample_'+str(sample_number)
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = skeletal_input_hybrid(input_shape)
        model.summary()
