...
for sample_number in sample_number_list:
    ...
    # Model architecture
    def dual_input_hybrid(input_shape):
        # First input: Skeletal images
        skeleton_input = Input(shape=input_shape, name="skeleton_input")
        x1 = TimeDistributed(Conv2D(32, (3, 3), activation="relu"))(skeleton_input)
        ...
        # Second input: Silhouette images
        silhouette_input = Input(shape=input_shape, name="silhouette_input")
        x2 = TimeDistributed(Conv2D(32, (3, 3), activation="relu"))(silhouette_input)
        x2 = TimeDistributed(MaxPooling2D((2, 2)))(x2)
        x2 = TimeDistributed(Conv2D(64, (3, 3), activation="relu"))(x2)
        x2 = TimeDistributed(MaxPooling2D((2, 2)))(x2)
        ...
        model = Model(inputs=[skeleton_input, silhouette_input], outputs=output)
    ...
    multimodal_path_sample = core_path+'Test-deep_learn/multi_modal/sample_'+str(sample_number)
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = dual_input_hybrid(input_shape)
