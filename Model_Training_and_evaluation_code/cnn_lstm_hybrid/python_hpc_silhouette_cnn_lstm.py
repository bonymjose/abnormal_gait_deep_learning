...
for sample_number in sample_number_list:
    ...
    for class_name, patho_folder_names in class_dirs.items():
        for folder in patho_folder_names:
            ...
            for participant_number_folder in sorted(os.listdir(folder_dir)):
                ...
                # Access the 'silhouette' participant_number_folders
                ...
                for image_folder_name in os.listdir(silhouette_view_dir):
                    ...
                    image_counter = 0  # Single counter for image types
    ...
    # Model architecture
    def silhouette_input_hybrid(input_shape):
        # Input Layer
        silhouette_input = Input(shape=input_shape, name='silhouette_input')
    
        # TimeDistributed wrapper to apply the same CNN to each time step in the sequence
        x = TimeDistributed(Conv2D(32, (3, 3), activation='relu'))(silhouette_input)
        ...
        # Create Model
        model = Model(inputs=silhouette_input, outputs=output)
    ...
    silhouette_path_sample = core_path+'Test-deep_learn/silhouette/sample_'+str(sample_number)
    model_save_location = silhouette_path_sample+'/normal_boosted_models/'  # specify the path
    Result_location=silhouette_path_sample+'/normal_boosted_Results/'

    # Ensure the directory exists, if not, create it
    if not os.path.exists(silhouette_path_sample):
        os.makedirs(silhouette_path_sample)
