...
for sample_number in sample_number_list:
    ...
    # Model architecture
    def silhouette_input_lstm(input_shape):
        # Input layer
        silhouette_input = Input(shape=input_shape, name='silhouette_input')

        # Reshape layer
        x = Reshape((-1, input_shape[1]*input_shape[2]*input_shape[3]))(silhouette_input)
    ...
    silhouette_path_sample = core_path+'Test-deep_learn/lstm_silhouette/sample_'+str(sample_number)
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = silhouette_input_lstm(input_shape)
