...
for sample_number in sample_number_list:
    ...
    # Model architecture
    def dual_input_lstm(input_shape):
        # First input: Skeleton
        skeleton_input = Input(shape=input_shape, name='skeleton_input')
        x1 = Reshape((-1, input_shape[1]*input_shape[2]*input_shape[3]))(skeleton_input)
        ...
        x2 = Reshape((-1, input_shape[1]*input_shape[2]*input_shape[3]))(silhouette_input)
    ...
    multimodal_path_sample = core_path+'Test-deep_learn/lstm_multi_modal/sample_'+str(sample_number)
    ...
    for fold_number, (train, val) in enumerate(kfold.split(np.zeros(len(labels_train)), labels_train)):
        ...
        model = dual_input_lstm(input_shape)
