...
image_count_limit_list=[10,15,20,25,30,35,40,45,50]
            ...
            with open(result_file, "w") as f:
                ...
                for model_name in os.listdir(model_dir):
                    if model_name.endswith('.h5'):
                        ...
                        predicted_probs = model.predict([skeleton_images, silhouette_images])
                        ...
                        plt.show()
                        print(f"GIST Confusion matrix for model_name {model_name} saved at {plot_filename}")
                        ...
                        plt.title(f'Receiver Operating Characteristic for Model {model_name}')
                        plt.legend(loc="lower right")

                        # Save the ROC curve
                        roc_filename = os.path.join(result_dir, f'gist_roc_curve_model_name_{model_name}.png')
                        plt.savefig(roc_filename)
                        plt.show()
                        print(f"GIST ROC curve for model_name {model_name} saved at {roc_filename}")
